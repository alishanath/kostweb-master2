{{-- <button type="button" class="bg-red-500 text-white p-2 rounded-full hover:bg-red-600"
    ...
    <div class="bg-white p-6 rounded-lg shadow-md w-1/3">
        ...
        <div class="flex justify-end space-x-5 mt-4">
            <button type="button" class="bg-gray-300 text-white px-4 py-2 rounded-full hover:bg-gray-600"
                onclick="closeDeleteModal({{ $item->id }})">Batal</button>
            <form action="{{ route('kamar.destroy', $item->id) }}" method="POST" class="inline-block">
                @csrf
                @method('DELETE')
                <button type="submit"
