...
class AuthController extends Controller
{
    ...
    public function prosesBooking(Request $request)
    {
        ...
        try {
            ...
            $tanggalInputEn = str_replace($bulanIndonesia, $bulanInggris, $request->start_date);
            $booking = new KelolaPemesanan();
            $booking->penghuni_id = auth()->id();
            $booking->kamar_id = $request->room_id;
            $booking->tanggal_sewa = \Carbon\Carbon::createFromFormat('d F Y', $tanggalInputEn)->format('Y-m-d');
            $booking->bukti_pembayaran = $request->total_amount;
            $booking->status = 'Menunggu'; // atau status awal lainnya
            $booking->save();
